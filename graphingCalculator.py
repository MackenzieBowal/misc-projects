# This program uses user input to draw the x and y axes and draw
# a function. The program loops to ask for expressions until the user enters a
# blank input.
...
def screenCoor(xo, yo, ratio, x, y):
    # Equations taken from Assignment 02 information sheet
    screenX = xo + ratio*x
    screenY = yo + ratio*y
...
#
#  Draw a circle with radius 5 around the point where the turtle cursor currently is
#
#  Usage -> drawCircle(pointer, "purple")
#
#  Parameters:
#   pointer: the turtle drawing object
#  color: the desired colour of the circle
#
#  Returns: nothing
#

def drawCircle(pointer, color):

    # Setting radius to 5
    radius = 5

    # Positioning the pointer to draw the circle:
    heading = 180
    currentHeading = pointer.heading()
    pointer.penup()
    pointer.setheading(heading)
    pointer.forward(radius)
    pointer.setheading(currentHeading)
    pointer.pendown()
    ...
    # Returning the pointer back to its starting location:
    pointer.setheading(heading)
    pointer.penup()
    pointer.backward(radius)
    pointer.setheading(currentHeading)
    pointer.pendown()
...
def maximum(pointer, y, currentColor):

    drawCircle(pointer, "purple")
    pointer.pencolor(currentColor)


def minimum(pointer, y, currentColor):

    drawCircle(pointer, "orange")
    pointer.pencolor(currentColor)
...
def drawExpr(pointer, xo, yo, ratio, xmin, xmax, expr):
    ...
    # and going there:
    pixelX, pixelY = screenCoor(xo, yo, ratio, x, yNow)
    ...
    while x <= xmax:
        ...
        pixelX, pixelY = screenCoor(xo, yo, ratio, x, yNow)
        pointer.goto(pixelX, pixelY)
        currentColor, _ = pointer.color()
       
    # Loop to scan through y-values and mark maxes and mins,
    # and find global max and min:
    ...
    while x <= xmax:
        ...
        if yBefore < yNow and yNow > yNext:
            localMax = maximum(pointer, yNow, currentColor)
           

        if yBefore > yNow and yNow < yNext:
            localMin = minimum(pointer, yNow, currentColor)
